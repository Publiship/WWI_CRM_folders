...
public partial class despatch_note_search : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!isLoggedIn())
        {
            Response.Redirect("../user_accounts/signin.aspx?" + "rp=" + wwi_security.EncryptString("despatch_notes/despatch_note_search", "publiship"));
        }
    ...
    #region databinding
    /// <summary>
    /// simple binding to table using DAL
    /// </summary>
    protected void bind_data_grid()
    {
        string[] _cols = { "despatch_id", "despatch_ref", "created_by", "created_date" };
        string[] _order = { "despatch_ref" };

        DataTable _dt = new Select(_cols).From(DAL.Logistics.Tables.DespatchNote)
            .Where(DAL.Logistics.DespatchNote.DespatchRefColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];

        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "despatch_id";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        string _url = "";
        string[] _args = { "", "", ""};
        string[] _refs = { "despatch_ref" };
        //primary key number 
        _args[0] = wwi_security.EncryptString(_grid.GetRowValues(e.VisibleIndex, _grid.KeyFieldName).ToString(), "publiship");
        //get despatch reference
        _args[1] = _grid.GetRowValues(e.VisibleIndex, _refs).ToString();

        switch (e.ButtonID.ToString())
        {
            case "cmdNew": //view only mode
                {
                    _args[2] = "Insert";
                    break;
                }
            case "cmdView": //view only mode
                {
                    _args[2] = "ReadOnly";
                    break;
                }
            case "cmdEdit": //edit mode
                {
                    _args[2] = "Edit";
                    break;
                }
        ...
        //open container form
        _url = string.Format("../despatch_notes/despatch_note.aspx?pid={0}&ref={1}&mode={2}", _args);
