...
public partial class employee_search : System.Web.UI.Page
{
    ...
    {
        string[] _cols = { "EmployeesTable.EmployeeID", "EmployeesTable.Name", "EmployeesTable.EmailAddress", 
                              "EmployeesTable.Live", "Departments.Department", "OfficeTable.OfficeName" };

        string[] _order = { "EmployeesTable.Name" };

        DataTable _dt = new Select(_cols).From(DAL.Logistics.Tables.EmployeesTable)
           .LeftOuterJoin(DAL.Logistics.Department.DepartmentIDColumn, DAL.Logistics.EmployeesTable.DepartmentIDColumn)
           .LeftOuterJoin(DAL.Logistics.OfficeTable.OfficeIDColumn, DAL.Logistics.EmployeesTable.OfficeIDColumn)
            .Where(DAL.Logistics.EmployeesTable.NameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];
        

        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "EmployeeID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/employee.aspx?pid={0}&mode={1}", _args);
