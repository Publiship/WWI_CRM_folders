...
public partial class port_search : System.Web.UI.Page
{
    ...
    {
        string[] _cols2 = { "PortTable.PortID", "PortTable.PortName", "CountryTable.CountryName" };
        string[] _order = { "PortTable.PortName" };

        DataTable _dt = new Select(_cols2).From(DAL.Logistics.Tables.PortTable)
            .LeftOuterJoin(DAL.Logistics.CountryTable.CountryIDColumn, DAL.Logistics.PortTable.CountryIDColumn)
            .Where(DAL.Logistics.PortTable.PortNameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];
        
        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "PortID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/port.aspx?pid={0}&mode={1}", _args);
