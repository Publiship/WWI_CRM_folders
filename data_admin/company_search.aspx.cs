...
public partial class company_search : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!isLoggedIn())
        {
            Response.Redirect("../user_accounts/signin.aspx?" + "rp=" + wwi_security.EncryptString("data_admin/company_search", "publiship"));
        }
    ...
    {
        string[] _cols = { "NameAndAddressBook.CompanyID", "NameAndAddressBook.CompanyName", "NameAndAddressBook.Address1", 
                             "NameAndAddressBook.Address2", "NameAndAddressBook.Address3", "NameAndAddressBook.PostCode", 
                             "NameAndAddressBook.RecordAddedBy", "NameAndAddressBook.TelNo", "NameAndAddressBook.FaxNo","CountryTable.CountryName" };
        string[] _order = { "NameAndAddressBook.CompanyName" };

        DataTable _dt = new Select(_cols).From(DAL.Logistics.Tables.NameAndAddressBook)
            .LeftOuterJoin(DAL.Logistics.CountryTable.CountryIDColumn, DAL.Logistics.NameAndAddressBook.CountryIDColumn)
            .Where(DAL.Logistics.NameAndAddressBook.CompanyNameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];

        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "CompanyID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/company.aspx?pid={0}&mode={1}", _args);
