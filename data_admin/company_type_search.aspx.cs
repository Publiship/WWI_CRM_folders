...
public partial class company_type_search : System.Web.UI.Page
{
    ...
    {
        string[] _cols2 = { "TypeTable.TypeID", "TypeTable.TypeName" };
        string[] _order = { "TypeTable.TypeName" };

        DataTable _dt = new Select(_cols2).From(DAL.Logistics.Tables.TypeTable)
            .Where(DAL.Logistics.TypeTable.TypeNameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];
        
        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "TypeID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/company_type.aspx?pid={0}&mode={1}", _args);
