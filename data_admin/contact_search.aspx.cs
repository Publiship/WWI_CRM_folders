...
public partial class contact_search : System.Web.UI.Page
{
    ...
    {
        string[] _cols2 = { "ContactTable.ContactID", "ContactTable.ContactName", "ContactTable.ContactInitials", "ContactTable.EMail", "ContactTable.OrderAck", "NameAndAddressBook.CompanyName" };
        string[] _order = { "ContactTable.ContactName" };

        DataTable _dt = new Select(_cols2).From(DAL.Logistics.Tables.ContactTable)
            .LeftOuterJoin(DAL.Logistics.NameAndAddressBook.CompanyIDColumn, DAL.Logistics.ContactTable.CompanyIDColumn)
            .Where(DAL.Logistics.ContactTable.ContactNameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];
        
        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "ContactID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/contact.aspx?pid={0}&mode={1}", _args);
