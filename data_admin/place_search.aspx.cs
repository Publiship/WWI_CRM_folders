...
public partial class place_search : System.Web.UI.Page
{
    ...
    {
        string[] _cols2 = { "PlacesTable.PlaceID", "PlacesTable.PlaceName", "CountryTable.CountryName" };
        string[] _order = { "PlacesTable.PlaceName" };

        DataTable _dt = new Select(_cols2).From(DAL.Logistics.Tables.PlacesTable)
            .LeftOuterJoin(DAL.Logistics.CountryTable.CountryIDColumn, DAL.Logistics.PlacesTable.CountryIDColumn)
            .Where(DAL.Logistics.PlacesTable.PlaceNameColumn).IsNotNull().OrderAsc(_order).ExecuteDataSet().Tables[0];
        
        this.dxgridSearch.DataSource = _dt;
        this.dxgridSearch.KeyFieldName = "PlaceID";
    ...
    protected void dxgridSearch_CustomButtonCallback(object sender, ASPxGridViewCustomButtonCallbackEventArgs e)
    {
        ...
        //open container form
        _url = string.Format("../data_admin/place.aspx?pid={0}&mode={1}", _args);
