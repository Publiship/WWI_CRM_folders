...
public partial class not_shipped_tracking : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        try
        {
            ...
            this.LinqServerModeDeliveriesNotShipped.Selecting += new EventHandler<DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs>(LinqServerModeDeliveriesNotShipped_Selecting);
    ...
    /// <param name="e"></param>
    protected void LinqServerModeDeliveriesNotShipped_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
    {
        ...
        //****************
        //int? _status = wwi_func.vint(wwi_func.lookup_xml_string("xml\\parameters.xml", "name", "NotShipped", "value"));
        //DateTime? _ets = wwi_func.vdatetime(wwi_func.lookup_xml_string("xml\\parameters.xml", "name", "startETS", "value"));
        //****************
        //get starting exworks date for not shipped from xml file (which makes it easy to change if necessary)
        //not shipped tracking uses ex-works date not ETS as we won't have the ETS until vessel ships
        DateTime? _exworks = wwi_func.vdatetime(wwi_func.lookup_xml_string("xml\\parameters.xml", "name", "startExWorks", "value"));
        //****************
        //get current status id for deliveries from xml file (which makes it easy to change if necessary)
        ...
        if (!string.IsNullOrEmpty(_query))
        {
            ...
            var _nquery = new linq.linq_view_deliveries_not_shippedDataContext().view_deliveries_by_exworks_no_vessel(_exworks, _jobclosed).Where(_query); //c => c.CompanyID == 7
            e.QueryableSource = _nquery;
                        
            //Int32 _count = _nquery.Count();

            if (!String.IsNullOrEmpty(_name)) { append_to_query_log(_query, _name);
            this.gridNotShipped.SettingsText.Title = "Search results: " + _name;
            }
        }
        else //default to display nothing until page is loaded 
        {
            ...
            var _nquery = new linq.linq_view_deliveries_not_shippedDataContext().view_deliveries_by_exworks_no_vessel(_exworks, _jobclosed).Where(c => c.OrderNumber == -1); //c => c.CompanyID == 7
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void gridNotShipped_DataBound(object sender, EventArgs e)
    {
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void gridNotShipped_CustomUnboundColumnData(object sender, ASPxGridViewColumnDataEventArgs e)
    {
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void gridNotShipped_HtmlRowCreated(object sender, ASPxGridViewTableRowEventArgs e)
    {
        if (e.RowType != DevExpress.Web.ASPxGridView.GridViewRowType.Data) return;

        int _docs = e.GetValue("document_folder") != null? (int)e.GetValue("document_folder"): 0;
        DevExpress.Web.ASPxEditors.ASPxButton _obj = this.gridNotShipped.FindRowCellTemplateControl(e.VisibleIndex, null, "dxbtnDocs") as DevExpress.Web.ASPxEditors.ASPxButton;
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void gridNotShipped_RowCommand(object sender, ASPxGridViewRowCommandEventArgs e)
    {
        try
        {
            ...
            string _orderno = this.gridNotShipped.GetRowValues(_idx, "OrderNumber") != null ? this.gridNotShipped.GetRowValues(_idx, "OrderNumber").ToString() : "";
            //100112 check to see if this order is attached to a document folder other than it's own ordernumber
            string _docfolder = this.gridNotShipped.GetRowValues(_idx, "document_folder") != null ? this.gridNotShipped.GetRowValues(_idx, "document_folder").ToString() : "";
            //160112 pass house BL as we can use this to ientify which orders are grouped together 
            string _housebl = this.gridNotShipped.GetRowValues(_idx, "HouseBLNUmber") != null ? this.gridNotShipped.GetRowValues(_idx, "HouseBLNUmber").ToString() : "";
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void gridNotShipped_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
    {
    ...
    protected void get_selected_rows(Int32 batchcommand)
    {
       ...
       List<Object> _orderids =  this.gridNotShipped.GetSelectedFieldValues(_fields);
    ...
    protected void set_visible_data(string cid)
    {
        try
        {
            ...
            if (File.Exists(_path))
            {
                ...
                this.gridNotShipped.SettingsCookies.Enabled = false;
                ...
                if (_dv.Count > 0)
                {
                    ...
                    for (int ix = 0; ix < _dv.Count; ix++)
                    {
                        ...
                        if (this.gridNotShipped.Columns[_fieldname] != null)
                        {
                            if (_visibleindex > -1)
                            {
                                this.gridNotShipped.Columns[_fieldname].VisibleIndex = _visibleindex;
                            }

                            this.gridNotShipped.Columns[_fieldname].Width = _width;
                            this.gridNotShipped.Columns[_fieldname].Visible = _visible; //= false will drop column into field choose
                        }
                    }
    ...
    protected void append_to_query_log(string _query, string _name)
    {
        try
        {
            if (!string.IsNullOrEmpty(_query))
            {
                ...
                _newlog.QrySource = "not_shipped";//"NSHIP";//_name.Contains("___report") ? "OTR" : "OTV"; ///not shipped
    ...
    protected string tabulate_columns_to_xml(string companyid)
    {
        ...
        try
        {
            ...
            for (int ix = 0; ix < this.gridNotShipped.Columns.Count; ix++)
            {
                string[] _values = {this.gridNotShipped.Columns[ix].Name.ToString(), this.gridNotShipped.Columns[ix].VisibleIndex.ToString(), 
                                   this.gridNotShipped.Columns[ix].Width.ToString(), this.gridNotShipped.Columns[ix].Visible.ToString() };
    ...
    protected void bind_linq_datasource_non_server_mode()
    {
        ...
        if (_filter != "")
        {
            ...
            this.gridNotShipped.DataSource = _query;
        }
        else
        {
    ...
    protected void remove_grid_grouping()
    {
        try
        {
            for (int i = 0; i < this.gridNotShipped.Columns.Count; i++)
                if (this.gridNotShipped.Columns[i] is GridViewDataColumn)
                {
                    GridViewDataColumn gridViewDataColumn = (GridViewDataColumn)this.gridNotShipped.Columns[i];
                    if (gridViewDataColumn.GroupIndex > -1)
                        this.gridNotShipped.UnGroup(gridViewDataColumn);
                }
        }
        catch (Exception ex)
        {
    ...
    protected void set_fixed_cols_resx(string resxkey)
    {
        ...
        uTrackingPrefs[] _prefs = new uTrackingPrefs[this.gridNotShipped.Columns.Count];
        
        //enum grid columns
        for (int c = 0; c <= this.gridNotShipped.Columns.Count - 1; c++)
        {
            _prefs[c].colName = this.gridNotShipped.Columns[c].Name;
            _prefs[c].visible = this.gridNotShipped.Columns[c].Visible;
            _prefs[c].visibleindex = this.gridNotShipped.Columns[c].VisibleIndex;
        }
    ...
    protected bool get_fixed_cols_resx(string resxkey)
    {
        ...
        {   
            ...
            if (_prefs.Count > 0)
            {
                ...
                {
                    DevExpress.Web.ASPxGridView.GridViewColumn _col = this.gridNotShipped.Columns[_prefs[c].colName];
                    if (_col != null)
                    {
            ...
            case "mnuDetail": //replaces show/hide details button
                {
                    ...
                    if (e.Item.Text == "Show detail")
                    {
                        ...
                        this.gridNotShipped.DetailRows.ExpandAllRows();
                    }
                    else
                    {
                        e.Item.Text = "Show detail";
                        this.gridNotShipped.DetailRows.CollapseAllRows();
                    }
                    break;
                }
