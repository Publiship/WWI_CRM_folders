...
public partial class shipment_tracking : System.Web.UI.Page
{
    ...
    protected void gridOrder_DataBound(object sender, EventArgs e)
    {
        ...
        {
            this.gridOrder.Columns[_hidden[_ix]].VisibleIndex = -1; //visibleindex=-1 should automatically set visible=false
        }
    ...
    protected void gridOrder_RowCommand(object sender, ASPxGridViewRowCommandEventArgs e)
    {
        try
        {
            ...
            string _orderno = this.gridOrder.GetRowValues(_idx, "OrderNumber") != null ? this.gridOrder.GetRowValues(_idx, "OrderNumber").ToString() : "";
            //100112 check to see if this order is attached to a document folder other than it's own ordernumber
            string _docfolder = this.gridOrder.GetRowValues(_idx, "document_folder") != null ? this.gridOrder.GetRowValues(_idx, "document_folder").ToString() : "";
            //160112 pass house BL as we can use this to ientify which orders are grouped together 
            string _housebl = this.gridOrder.GetRowValues(_idx, "HouseBLNUmber") != null ? this.gridOrder.GetRowValues(_idx, "HouseBLNUmber").ToString() : "";
            ...
            if (!string.IsNullOrEmpty(_orderno))
            {
                switch (_command)
                {
                    case "link_filemanager":
                        {
                            //internal use only!
                            string _companyid = Session["user"] != null ? ((UserClass)Session["user"]).CompanyId.ToString() : "0";
                            if (_companyid == "-1")
                            {
    ...
    protected void append_to_query_log(string _query, string _name)
    {
        try
        {
            if (!string.IsNullOrEmpty(_query))
            {
                ...
                _newlog.QrySource = "shipment"; //deprecated 20/03/15 _name.Contains("___report") ? "OTR" : "OTV"; ///order tracking REPORT (goes to combobox) or order tracking view
    ...
    protected void bind_combos()
    {
        ...
        if (Session["user"] != null)
        {
            ...
            if (_companyid != -1) //bind contacts for this company
            {
                ...
                Query _qry = new Query(Tables.ContactTable, "WWIprov").
                    WHERE("CompanyID", Comparison.Equals, _companyid)
                    .AND("Live", Comparison.Equals, true)  
    ...
    protected string tabulate_columns_to_xml(string companyid)
    {
        ...
        try
        {
            ...
            for (int ix = 0; ix < this.gridOrder.Columns.Count; ix++)
            {
                string[] _values = {this.gridOrder.Columns[ix].Name.ToString(), this.gridOrder.Columns[ix].VisibleIndex.ToString(), 
                                   this.gridOrder.Columns[ix].Width.ToString(), this.gridOrder.Columns[ix].Visible.ToString() };
        ...
        if (this.dxcbofields.Text.ToString() != string.Empty && this.txtQuickSearch.Text.ToString() != string.Empty)
            {
                ...
                if (_fieldtype.ToLower() == "string")
                {
                    ...
                    if (Page.Session["user"] != null)
                    {
                        ...
                        _criteria = ".StartsWith("; //".Contains(" is too slow
                        _rquoted = "\")"; //"\")";
                    }
                }
