...
public partial class file_manager_all : System.Web.UI.Page
{
    ...
    protected void set_visible_folder(bool iseditor, string orderno, string docfolder)
    {
        ...
        //************ this coding is different to the same section in ord_edit_files_1.aspx so we can see all folders ***********
        //
        if (_dir)
        {
            if (iseditor) //need to replace this filemanager with somthing more adaptable! you can't switch on/off folder tree once page is loaded! rubbish 
            {
                ...
                //290113 initial folcder is very slow as it loads all folder names to directory tree before displaying. Find a better component
        ...
        else
        {
            if (iseditor)
            {
                this.dxfmpod.Settings.InitialFolder = "not found";
    ...
    protected int file_transfer(string sourcePath, string targetPath, bool overWrite)
    {
        ...
        {
            this.dxlblerr.Text = "Unable to copy files from temporary path to target path";
            this.dxpnlerr.Visible = true;
        }
    ...
    protected static void adapter_RowUpdating(object sender, SqlRowUpdatingEventArgs args)
    {
    ...
    protected void dxbtnfolders_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("file_manager.aspx?pod=" + _orderno + "&cd=" + _cdir + "&hbl=" + _housebl); 
    }
